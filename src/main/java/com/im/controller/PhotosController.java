package com.im.controller;

import com.im.api.*;
import com.im.lib.annotation.WebsocketHandler;
import com.im.lib.annotation.WebsocketHandlerMapping;

@WebsocketHandler
public class PhotosController {

	@WebsocketHandlerMapping(value = 0x72d4742c, name = "UpdateProfilePhoto")
	public PhotosApi.Photo updateProfilePhoto(PhotosApi.UpdateProfilePhoto updateProfilePhoto) {
		return null;
	}

	@WebsocketHandlerMapping(value = 0x89f30f69, name = "UploadProfilePhoto")
	public PhotosApi.Photo uploadProfilePhoto(PhotosApi.UploadProfilePhoto uploadProfilePhoto) {
		return null;
	}

	@WebsocketHandlerMapping(value = 0x87cf7f2f, name = "DeletePhotos")
	public long[] deletePhotos(PhotosApi.DeletePhotos deletePhotos) {
		return null;
	}

	@WebsocketHandlerMapping(value = 0x91cd32a8, name = "GetUserPhotos")
	public PhotosApi.Photos getUserPhotos(PhotosApi.GetUserPhotos getUserPhotos) {
		return null;
	}
}